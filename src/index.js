  ...
  useEffect(() => {
    ...
    unsubs.push(
      onSnapshot(
        ...
        (snap) => setSentConnectionRequests(snap.docs.map(d => d.id))
      )
    );
    ...
    unsubs.push(
      onSnapshot(
        ...
        (snap) => setBlockedUsers(snap.docs.map(d => d.id))
      )
    );
            ...
            {activeTab === 'messages' && (
              <div className={isNarrowScreen ? "space-y-4" : "grid md:grid-cols-12 gap-6"}>
                {(!isNarrowScreen || !selectedChat) && (
                  <div className="md:col-span-4 lg:col-span-3 bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm">
                ...
                {(!isNarrowScreen || selectedChat) && (
                  <div className={`md:col-span-8 lg:col-span-9 bg-white dark:bg-slate-800 rounded-2xl shadow-sm flex flex-col ${isNarrowScreen ? 'h-[70vh]' : 'h-[80vh]'}`}>
                    {selectedChat ? (
                      <>
