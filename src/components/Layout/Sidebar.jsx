...
function Sidebar({ collapsed, onToggleCollapse, isMobileOpen, onMobileClose }) {
  ...
  // Desktop Sidebar - No fixed positioning, just normal flow
  const DesktopSidebar = () => (
    <div 
      className={`h-screen bg-white dark:bg-slate-900 transition-all duration-300 flex flex-col border-r border-slate-200 dark:border-slate-800 ${
        collapsed ? 'w-20' : 'w-64'
      }`}
    >
      {/* Logo Section */}
      ...
      <button
        ...
        className="absolute -right-3.5 top-1/2 w-7 h-7 bg-white dark:bg-slate-800 rounded-full shadow-lg flex items-center justify-center border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all"
      >
