// UploadData.jsx — Material Upload Page with Category Selection
...
function UploadsData() {
  const [step, setStep] = useState(1); // 1: category, 2: details, 3: upload
  ...
  const categories = [
    { value: 'Past Questions', icon: ScrollText, label: 'Past Questions', color: 'from-amber-500 to-orange-600' },
    { value: 'PDF Notes', icon: FileText, label: 'PDF Notes', color: 'from-blue-500 to-cyan-600' },
    { value: 'Video Tutorials', icon: Video, label: 'Video Tutorials', color: 'from-purple-500 to-pink-600' },
    { value: 'Technical Reviews', icon: BookOpen, label: 'Technical Reviews', color: 'from-emerald-500 to-teal-600' },
  ];
  ...
  const handleUpload = () => {
    ...
    setTimeout(() => {
      ...
      // In real app, save to backend or Firebase
      // Here we just update localStorage for demo sync
      const existing = JSON.parse(localStorage.getItem('uploadedMaterials') || '[]');
      existing.unshift(newMaterial);
      localStorage.setItem('uploadedMaterials', JSON.stringify(existing));
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {categories.map((cat) => (
                <button
                  ...
                  className="group relative p-8 rounded-3xl bg-white dark:bg-slate-800 shadow-xl hover:shadow-2xl transition-all duration-500 border border-slate-200/50 dark:border-slate-700/50 overflow-hidden"
                >
                  ...
                  <div className={`w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br ${cat.color} flex items-center justify-center text-white shadow-xl group-hover:scale-110 transition-transform`}>
                    <cat.icon size={40} />
                  </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 md:p-12">
            ...
            <div className="space-y-6">
              <input
                ...
                className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-500/30 transition-all text-lg"
              />
              ...
              <input
                ...
                onChange={handleInputChange}
                placeholder="Course Name & Code (e.g., PTE411 Pipeline Technology)"
                className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-500/30 transition-all text-lg"
              />
            ...
            <div className="mb-8">
              <label className="block">
                <input
                  ...
                  className="hidden"
                />
                <div className={`border-4 border-dashed rounded-3xl p-12 cursor-pointer transition-all hover:border-purple-500 ${
                  ...
                  {formData.preview ? (
                    <div className="space-y-4">
                      <div className="w-32 h-32 mx-auto bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl flex items-center justify-center text-white text-4xl font-bold">
                        {formData.file.name.slice(-3).toUpperCase()}
                      </div>
